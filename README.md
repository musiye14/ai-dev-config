...
### D. MCP(外部能力,详见 mcp.md)

| 功能 | 作用 | 依赖 | 备注 |
|---|---|---|---|
| **D1 Context7** | 查询当前版本的库和框架文档 | Node >=18 | 已存在则跳过 |
| **D2 Playwright** | 浏览器操作、页面检查和端到端验证 | Node >=18 + 可用浏览器 | 使用 Microsoft 官方 MCP |
...
1. **先展示清单,不要安装**。首轮完整展示上方 A1-D2 功能,每项包含编号、名称、作用和当前状态(`已安装` / `可安装` / `不支持`)；让用户回复要安装的编号(可多选或全选)。展示后必须暂停并等待用户选择,不得默认全装,也不得提前复制文件、修改配置或执行安装命令。
2. **收到明确选择后再安装**。复述用户选择,跳过已安装项,只处理用户选中且缺失的功能；用户未选择的项目保持不变。
3. 查 `cache/<tool>/<ver>/` 是否命中:
   - **命中** → 按用户选择复制对应文件,收工。
   - **没命中** → 继续下面。
4. 探查工具能力(hook / statusLine / skill 目录 / instructions 文件 / MCP 配置和已有服务)。
5. 按用户勾选逐项落地:
   ...
   - **D 类**:读 mcp.md,先按名称和能力探测已有 MCP,只安装缺失项;不覆盖已有认证和参数。
   - 工具做不到的能力 → 豁免,不硬塞降级。
6. 部署完汇报`已安装`、`已跳过`和`安装失败`清单,让用户验证；可靠后存快照到 `cache/<tool>/<ver>/`。

### 加 / 删功能

- **加习惯规则** → 改 `features.md` 加一节 + 本 README 功能清单加一行。
- **加插件** → 改 `registry.md` 加一条 + 本 README 功能清单加一行。
- **加 MCP** → 改 `mcp.md` 加一条 + 本 README 功能清单加一行。
- **删** → 删对应节/行。
